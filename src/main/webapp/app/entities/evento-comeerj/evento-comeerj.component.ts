...
@Component({
    selector: 'jhi-evento-comeerj',
    templateUrl: './evento-comeerj.component.html'
})
export class EventoComeerjComponent implements OnInit, OnDestroy {
...
currentAccount: any;
        ...
        if (this.currentSearch) {
            this.eventoService.search({
                ...
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<EventoComeerj[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/evento-comeerj'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/evento-comeerj', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: EventoComeerj) {
        return item.id;
    }
