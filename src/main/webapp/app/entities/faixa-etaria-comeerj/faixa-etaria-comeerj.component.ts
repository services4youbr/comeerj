...
@Component({
    selector: 'jhi-faixa-etaria-comeerj',
    templateUrl: './faixa-etaria-comeerj.component.html'
})
export class FaixaEtariaComeerjComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/faixa-etaria-comeerj'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: FaixaEtariaComeerj) {
        return item.id;
    }
    registerChangeInFaixaEtarias() {
        this.eventSubscriber = this.eventManager.subscribe('faixaEtariaListModification', (response) => this.loadAll());
    }
