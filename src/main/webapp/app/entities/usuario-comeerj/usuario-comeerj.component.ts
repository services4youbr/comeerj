...
@Component({
    selector: 'jhi-usuario-comeerj',
    templateUrl: './usuario-comeerj.component.html'
})
export class UsuarioComeerjComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.usuarioService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<UsuarioComeerj[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/usuario-comeerj'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/usuario-comeerj', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: UsuarioComeerj) {
        return item.id;
    }
